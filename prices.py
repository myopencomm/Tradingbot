...
# La TTF vit dans ttf.py (module feuille) : `config` avait besoin de cette
# seule fonction et importait `prices` pour l'obtenir, alors que `prices`
# importait `config` — cycle contourné par des imports différés des deux côtés.
def is_french_large_cap(ticker: str) -> bool:
    """Assujettissement à la TTF — source unique : ttf.py."""
    import ttf
