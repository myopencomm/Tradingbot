...
💶 **Titres en devise : deux % différents, tous deux justes.** Les seuils se jugent sur le cours en **devise** contre le PRU en devise : le stop est un ordre en dollars, déclenché par le cours de l'action. Le « var / PRU » de BD est en **euros** et inclut le change (JNJ le 24/09/2026 : +3,4 % en dollars, +5,6 % chez BD, le dollar ayant pris ~1,6 % depuis l'achat). `/trailing` affiche les deux, avec la part due au change (`position_view.eur_perf`, PRU BD brut frais inclus) ; `/status` et le dashboard chiffrent désormais le % et le P&L euros change compris, comme BD.
