...
| **Alerte news et résultats (alerte seule)** | Lundi et jeudi à 8h50 (`NEWS_ALERT_DAYS`, `NEWS_ALERT_TIME`), le bot lit les articles récents de chaque position gérée (hors HOLD et titres sans valeur) et envoie une alerte Telegram si l'un d'eux est une **mauvaise nouvelle pour le cours** : dégradation, avertissement, procès, rappel de produit, départ de dirigeant. Chaque titre d'article est jugé par **Jev (TypeSafe)**, qui donne deux probabilités : l'article parle-t-il bien de cette société, et est-il négatif pour son cours. L'alerte part si les deux dépassent 80 % (`NEWS_ALERT_THRESHOLD`). Seuls les articles jamais vus sont envoyés, un appel par position. Il prévient aussi **7 jours avant des résultats** (`EARNINGS_ALERT_DAYS`), une fois par date, parce qu'un stop-loss ne protège pas d'un écart à l'ouverture. **Aucune vente automatique** : le backtest du 23/09/2026 (`backtest.py --review`, section Backtest) montre que les sorties anticipées sur le prix font baisser le P&L, et les nouvelles sont la seule chose qu'il ne peut pas rejouer. Jev est fiable sur du texte mais pas sur des chiffres : il ne reçoit que des titres et résumés d'articles publics. Sans `TYPESAFE_API_KEY`, seule l'alerte résultats fonctionne. `/news` lance un tri immédiat |
| **Relevé BD sans cotation ignoré** | Une ligne du portefeuille BD dont la variation du jour est « - » (aucune cotation vivante) ne remplace plus le dernier relevé coté. Le 22/09/2026 à 22:35, BD a servi toutes les valeurs US avec un cours converti en EUR mais libellé USD : JNJ à « 234.53 USD » pour 269.19 réels, soit toutes les lignes US à environ -12 %. Le dashboard, qui préfère un relevé BD plus récent qu'une barre yfinance, affichait des pertes latentes qui n'existaient pas. Exception : les titres actés sans valeur (GVN, MCPHY), pour qui « - » est l'état permanent |
...
├── stale_exit.py            Vitesse des positions : verdict et mesures (vente coupee)
...
| `/news` | News négatives et résultats imminents sur les positions détenues — **alerte seule, jamais de vente**. Rend compte de chaque position (le passage planifié ne parle que s'il trouve quelque chose) — voir *Alerte news* dans les fonctionnalités |
...
| `/sync` | Synchroniser le portefeuille depuis Bourse Direct — détecte et clôture automatiquement les ventes exécutées (TP/SL touchés), ajoute les positions issues d'achats exécutés. Les messages d'étape sont **éphémères** (supprimés dès que le résultat s'affiche) ; ils ne restent que si le sync échoue. Si le contrôle de protection trouve une position à nu, **la repose est tentée dans la foulée** — constater sans réparer n'a pas de sens quand la réparation existe (01/09/2026 : `/sync` annonçait deux positions sans protection et attendait le cycle de 9h35) |
| `/trailing` | Forcer une vérification du trailing stop (SL au PRU) **maintenant**, avec le détail de chaque position et la raison d'un non-déclenchement. Pour un titre en devise, ajoute le % en euros vu par BD et la part due au change Le cycle automatique tourne chaque heure à **:35** (jours de marché, 9h-22h, session BD connectée) et dès qu'une position franchit son seuil aux checks 9h/12h/15h/17h — mais il reste silencieux s'il n'a rien à faire, contrairement à `/trailing` qui répond toujours |
| `/stagnation` | Verdict de **vitesse** sur chaque position : âge en jours de bourse, part du chemin PRU→TP parcourue, jalon applicable, et pour une position bloquée le cours qu'il lui faudrait pour sortir sans perte. **Constat seul** — la vente automatique sur stagnation est désactivée (`STALE_EXIT`) |
...
💶 **Titres en devise : le palier 1 se juge en euros.** Le « var / PRU » de BD est en euros et inclut le change (JNJ le 24/09/2026 : +3,4 % en dollars, +5,6 % chez BD, le dollar ayant pris ~1,6 % depuis l'achat). Le capital étant en euros, c'est **ce %-là** qui déclenche le breakeven (`trailing.breakeven_basis`), et le SL « au PRU » est le **PRU euros converti au taux du jour** — relevé au PRU en dollars s'il est plus haut et laisse de l'air sous le cours (protège aussi d'un repli du dollar). Le palier 2 reste jugé en devise, comme le TP. `/trailing` affiche les deux % avec la part due au change (`position_view.eur_perf`, PRU BD brut frais inclus) ; `/status` et le dashboard chiffrent désormais le % et le P&L euros change compris, comme BD.
...
| Règle (2023 → 09/2026) | 137 titres, 4 pos. | 137 titres, 2 pos. | 679 titres, 4 pos. |
|---|---|---|---|
| Référence (SL/TP/trailing seuls) | -1711 € | -754 € | -2923 € |
| MM50 cassée → vente, quotidien | -2412 € | -1260 € | -3718 € |
| MM50 cassée → vente, hebdo | -2267 € | -1093 € | -3513 € |
| MM50 cassée → SL remonté, quotidien | -1885 € | -847 € | -3165 € |
| MM50 cassée → SL remonté, hebdo | -1949 € | -741 € | -3062 € |
| Chandelier 2,5 ATR → vente | -1769 € | -821 € | -2992 € |
| Chandelier 4 ATR → vente | -1714 € | -754 € | -2944 € |
| Choc > 2 ATR → vente | -1706 € | -741 € | -2946 € |
