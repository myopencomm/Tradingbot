...
# ─── Budget, capacité, taille de position ───────────────────────────────────
# Ces fonctions vivent dans sizing.py : elles répondent à « ai-je le droit
# d'entrer, et pour combien ? », question que se posent aussi les scans
# présentés à l'utilisateur. C'est ce que `analysis` empruntait ici — et c'est
# ce qui faisait tourner en rond les deux modules.
import sizing
is_enabled            = sizing.is_enabled
get_budget_info       = sizing.get_budget_info
set_config            = sizing.set_config
compute_position_size = sizing.compute_position_size
entry_capacity_block  = sizing.entry_capacity_block
entry_blocked_reason  = sizing.entry_blocked_reason
...
# ─── Trailing stop ──────────────────────────────────────────────────────────
# Le trailing vit dans trailing.py : entrer en position et protéger une
# position gagnante sont deux métiers, et il pesait un tiers de ce module.
# Les noms restent exposés ici — main, monitor et telegram_bot les appellent.
import trailing
tp_progress         = trailing.tp_progress
trailing_target     = trailing.trailing_target
trailing_stop_cycle = trailing.trailing_stop_cycle
...
# ─── Enregistrement auprès d'analysis ───────────────────────────────────────
# `analysis` a besoin de DEUX actions du moteur : entrer immédiatement sur une
...
def _entrer_maintenant(send_fn) -> None:
    ...
    if not (is_enabled() and bot_mode.is_playwright()
        ...
        return
    threading.Thread(target=run_entry_cycle, args=(send_fn,), daemon=True).start()
